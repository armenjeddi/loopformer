...
# ----------------- Settings -----------------
OUTDIR      = "pile_dedup_tokens"                 # output folder
DATASET     = "EleutherAI/the_pile_deduplicated"  # dataset id
NAME        = "all"                               # config name (dataset has config "all")
SPLIT       = "train"                             # only train exists for this dataset
TEXT_FIELD  = "text"
...
# retry/skip knobs (short + strict)
RETRY_LIMIT_PER_SHARD = 3
...
def load_pile_dedup_stream():
    ...
    def _call(**kwargs):
        # token kwarg name differs across datasets versions
        try:
            return load_dataset(**kwargs, token=token)
        except TypeError:
            return load_dataset(**kwargs, use_auth_token=token)

    # try preferred NAME first, but fall back cleanly to default/no-name
    for delay in [0.5, 1.0, 2.0]:
        try:
            if NAME is None:
                return _call(path=DATASET, split=SPLIT, streaming=True)
            return _call(path=DATASET, name=NAME, split=SPLIT, streaming=True)
        except ValueError as e:
            if "BuilderConfig" in str(e) and "not found" in str(e):
                # your case: only "default" exists
                return _call(path=DATASET, split=SPLIT, streaming=True)
            raise
...
def main():
    ...
    # Stream dataset
    ds = load_pile_dedup_stream()

    # ---- Robust iteration with retry/skip on 429s ---------------------------
    from huggingface_hub.errors import HfHubHTTPError

    # Pile dedup parquet names look like:
    # train-01649-of-01650-c7ca9e12a36e8652.parquet
    PARQUET_RE = re.compile(r"(train-\d{5}-of-\d{5}-[0-9a-f]+\.parquet)")
    per_shard_tries = {}
    ...
    def try_shuffle(d):
        # IterableDataset.shuffle exists in many datasets versions; buffer_size helps.
        try:
            return d.shuffle(seed=(int(time.time()) & 0xFFFF), buffer_size=10_000)
        except Exception:
            return d
    ...
    doc_idx = -1  # last successfully processed doc index (0-based)
    ...
    while True:
        try:
            ...
            processed = doc_idx + 1  # number of docs already processed
            ...
            if tries < RETRY_LIMIT_PER_SHARD:
                ...
                dset = try_shuffle(ds).skip(processed)
                it = new_iter(dset)
                continue
            else:
